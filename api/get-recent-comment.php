...
// Get the most recent comment from any user
$result = $conn->query("SELECT c.*, u.name as user_name FROM lead_comments c LEFT JOIN users u ON c.user_id = u.id WHERE c.lead_id = $lead_id ORDER BY c.created_at DESC LIMIT 1");
